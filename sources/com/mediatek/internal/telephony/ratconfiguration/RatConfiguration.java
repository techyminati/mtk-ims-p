...
public class RatConfiguration {
    ...
    protected static synchronized int getMaxRat() {
        ...
        synchronized (RatConfiguration.class) {
            if (!max_rat_initialized) {
                String sMaxRat = SystemProperties.get(PROPERTY_BUILD_RAT_CONFIG, MtkPhoneConstants.MVNO_TYPE_NONE);
    ...
    protected static int getRatConfig() {
        ...
        if (default_rat_config == 0) {
            ...
            String rat = SystemProperties.get(PROPERTY_RAT_CONFIG, MtkPhoneConstants.MVNO_TYPE_NONE);
            if (rat.length() > 0) {
    ...
    protected static String ratToString(int iRat) {
        String rat = MtkPhoneConstants.MVNO_TYPE_NONE;
        if ((iRat & 32) == 32) {
            rat = rat + "/C";
        }
        if ((iRat & 16) == 16) {
